buglist_alpha_index_ui <- function(id) {
    ns <- NS(id)
    res <- div(
        fluidRow(
            column(3,
...
buglist_alpha_index_mod <- function(id, mpse) {
    moduleServer(
        id,
        function(input, output, session) {
            ...
            #calculate the alpha index
            mp_alpha <- eventReactive(input$btn, {
                req(inherits(mpse, "MPSE"))
                input$submit
                mp_alpha <- mp_cal_alpha(mpse, .abundance = Abundance, force = TRUE)
                return(mp_alpha)
            })
