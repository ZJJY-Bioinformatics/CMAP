...
alpha_mul_index_ui <- function(id) {
    ns <- NS(id)
    res <- div(
        #class = "tab-body",
        fluidRow(
...
alpha_mul_index_mod <- function(id, mpse) {
    moduleServer(
        id,
        function(input, output, session) {
            ...
            mp_mul_alpha <- eventReactive(input$btn, {
                req(inherits(mpse, "MPSE"))
                #input$submit
                mp_mul_alpha <- mp_cal_alpha(mpse, .abundance = RareAbundance, force = T)
                if (!is.null(treeda())) {
                    pd_a_index <- pd(
            ...
            p_mul_alpha_index <- reactive({
                ...
                #box_width <- isolate({input$box_width})
                #dodge <- isolate({input$dodge})
               ...
               if (input$point){
                   p <- ggplot(data, aes(x = !!sym(x_group), y = !!sym(index), color = !!sym(group))) + 
                       ...
                       #geom_point(position = position_jitter(0.2),size = 3, color = "black") + 
